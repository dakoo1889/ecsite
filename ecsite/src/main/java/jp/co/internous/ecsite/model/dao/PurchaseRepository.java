...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
	...
	@Query(value="SELECT * FROM purchase p " +  //purchaseテーブルの
				"WHERE created_at = (" +        //created_atカラムを抽出
				"SELECT MAX(created_at) FROM purchase p WHERE p.user_id = :userId) ", //created_atカラムの最大値を取得 条件:(=:変数名) ユーザーIDが一致
				nativeQuery=true)
